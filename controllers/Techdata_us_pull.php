...
class Techdata_us_pull extends CI_Controller {

	function __construct(){
        parent::__construct();
		$this->load->library('Emailtemplate');

		$this->data['instance_ip'] = 'http://13.59.150.81/php_td_us_pull/';
	...
	public function index(){
		...
		$this->load->view('techdata_us_pull',$data);
	}
	
	public function refresh_div_us(){
		$this->load->model('admin_dashboard_m');
		$get_all_sync_data_tdus = $this->admin_dashboard_m->get_all_sync_data_tdus();
		$json_encode = json_encode($get_all_sync_data_tdus,true);
		echo $json_encode;
	}

	public function send_email_tdus(){
	  
		$this->load->model('admin_dashboard_m');
		$fetch_records = $this->admin_dashboard_m->get_records_us();
		...
		if($fetch_records->email_status == '' && $fetch_records->status == 'Successfull')
		{
			...
			foreach($get_all_admin as $fetch_admin){
				...
				$message = $this->emailtemplate->td_us_daily_email_two($admin_name,$total_products);
				$this->load->library('email', $config);
				$this->email->from('noreply@protectbox.com','Protectbox');
				$this->email->to('sweezit92@gmail.com');
				$this->email->subject('TD US Data pulling complete');
				...
				if($send_email){
					...
					$this->email->from('noreply@protectbox.com','Protectbox');
					$this->email->to('sweezit92@gmail.com');
					$this->email->subject('TD US Active product select complete');
	...
	public function fetch_scrape_data(){
		...
		//$unscrape_data = $this->admin_dashboard_m->get_all_unscrape_data($service_provider);
		...
		public function fetch_scrapenew_data(){
		$this->load->model('admin_dashboard_m');
		$service_provider = 'TechData US';
		$get_all_scrape_data = $this->admin_dashboard_m->get_all_scrape_data($service_provider);
		$get_all_data = $this->admin_dashboard_m->total_services_admin_dash();
    ...
    public function tdus_scrapenew(){
		$url = 'http://18.221.137.102:5000/tdusnew';
		$ch = curl_init($url);
	...
	/************************** NOT USED *******************/
	/*public function tdus_pull(){
		$url = 'http://3.19.246.91/php_td_us_pull';
		$ch = curl_init($url);
		curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type:application/json'));
		curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
	...
	public function fetch_loader(){
		...
		$get_all_sync_data_tdus = $this->techdata_pull_m->get_all_sync_data_tdus();
		$json_encode = json_encode($get_all_sync_data_tdus,true);
		echo $json_encode;
	}
	...
	public function send_email_tdus(){
		$this->load->model('techdata_pull_m');
		$fetch_records = $this->techdata_pull_m->get_records_us();
		$records_id = $fetch_records->td_pull_records_id;
		$total_products = $fetch_records->total_records;
		...
		if($fetch_records->email_status == '' && $fetch_records->status == 'Successfull')
		{
			...
			foreach($get_all_admin as $fetch_admin){
				...
				$send_email = $this->email->send();
				if($send_email){
					$order_message = $this->emailtemplate->td_us_daily_email($admin_name,$totat_product_updated,$active_product,$new_product);
					...
					$this->email->attach('http://3.19.246.91/php_td_us_pull/unmapped/'.$fetch_records->xls_file_name.'');
					$smb_email = $this->email->send();
				}
				break;
			}
