...
class Completed_refund_supplier extends CI_Controller {
    ...
    {
		$this->load->model('completed_refund_supplier_m');
		$user_id = $this->session->userdata['logged_in']['user_id'];
		$status = "completed";
		//$service_records = array('refund_status'=> $status);
		$get_all_service = $this->completed_refund_supplier_m->fetch_all_serivce($user_id,$status);
		//print_r($get_all_service);
		//exit;
		//print_r($get_supplier_details);

		$data['completed_refund'] = $get_all_service;
		
  
		//$this->load->view('sales',$data);
		$this->load->view('completed_refund_supplier',$data);
