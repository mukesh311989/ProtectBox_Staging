  ...
  <body>
	...
	<script>
		...
		$(document).ready(function(){
			interval = setInterval(readTextFile,1000);
			$.ajax({
			  url: '<?php echo base_url();?>bundle_json/result_bundle',
			  type: "post",
			  success: function(response){
				 var redirect_result = "<?php echo base_url('results');?>";
				 window.location.href = redirect_result;
			  }
			});
		});
		...
		function readTextFile(){
			count = count + 1;
			var progress_percent = (count/1000)*100;
			var int_progress = parseInt(progress_percent);
			//alert(int_progress);
			var bal = int_progress+'%';
			$("div#myBar").width(bal);
